...
IF EXIST "%ICON%" (
	py -3 -m PyInstaller --clean --noconfirm --onefile --windowed --icon "%ICON%" -n "%OUTPUT_NAME%" packaging\windows\entry_point.py
) ELSE (
	py -3 -m PyInstaller --clean --noconfirm --onefile --windowed -n "%OUTPUT_NAME%" packaging\windows\entry_point.py
)
